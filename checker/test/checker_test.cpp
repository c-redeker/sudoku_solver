...
TEST_F(RectangleFixture, empty_sudoku__valid) {
    EXPECT_TRUE(SudokuChecker::IsSolvable(m_sudoku));
}

TEST_F(RectangleFixture, row_containing_different_numbers__valid) {
    m_sudoku.GetRow(3)->GetAllCells()[0]->SetNumber(5);
    m_sudoku.GetRow(4)->GetAllCells()[1]->SetNumber(7);
    EXPECT_TRUE(SudokuChecker::IsSolvable(m_sudoku));
}

TEST_F(RectangleFixture, row_column_containing_different_numbers__valid) {
    m_sudoku.GetColumn(7)->GetAllCells()[0]->SetNumber(5);
    m_sudoku.GetRow(1)->GetAllCells()[7]->SetNumber(7);
    EXPECT_TRUE(SudokuChecker::IsSolvable(m_sudoku));
}

TEST_F(RectangleFixture, row_containing_number_2_times__invalid) {
...
TEST_F(RectangleFixture, rectangle_containing_number_2_times__invalid) {
